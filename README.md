...
Laster konfigurasjon (API-URLer, headere, parametere) fra config.yaml
Henter værvarsel fra MET.no Locationforecast API
Henter luftkvalitetsvarsel fra MET.no Air Quality API
Validerer begge svar mot Pydantic-skjemaer
Sjekker at begge kildene gjelder samme lokasjon (koordinater innenfor 0.05° toleranse)
Behandler og slår sammen dataene på breddegrad/lengdegrad
Lagrer resultatet i en SQLite-database, hopper over duplikater
...
Kjøre lokalt
Krever Python 3.11+ og uv.
bashuv sync                    # installer avhengigheter
uv run python main.py      # kjør pipelinen
Tester
bashuv run python -m pytest tests/ -v
...
Loads configuration (API URLs, headers, params) from config.yaml
Fetches a weather forecast from the MET.no Locationforecast API
Fetches an air-quality forecast from the MET.no Air Quality API
Validates both responses against Pydantic schemas
Checks that both sources describe the same location (coordinates within 0.05° tolerance)
Processes and merges the data on latitude/longitude
Stores the result in a SQLite database, skipping duplicates
